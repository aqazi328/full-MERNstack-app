...
export default class EditBug extends Component{
    ...
    componentDidMount(){
        axios.get('http://localhost:5000/bugs/' + window.location.pathname.substr(6))
            .then(response =>{
                this.setState({
    ...
    onSubmit(e){
        ...
        axios.post('http://localhost:5000/bugs/update/' + window.location.pathname.substr(6), bug)
    ...
    render(){
        return(
            <div class="flex flex-col justify-center items-center">
                <h3 class="block text-gray-1000 font-bold md:text-right my-10 text-2xl">Edit Bug Form</h3>
                <form onSubmit={this.onSubmit} class="w-full max-w-sm">
                    ...
                    <div class="flex flex-col justify-center items-center ">
                        <input  class="shadow bg-teal-500 hover:bg-purple-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="submit" value="Edit Bug Form" />
                    </div>
                </form>
            </div>
        )
